...
import {
  ...
  initiatePaymentContextWithExistingCustomerStreamPayId,
  ...
  updatePaymentContextWithExistingCustomerStreamPayId,
...
describe("StreamPayTest", () => {
  describe("getPaymentStatus", function () {
    ...
    beforeAll(async () => {
      ...
      streampayTest = new StreamPayTest(scopedContainer, { api_key: "test" })
      await streampayTest.init()
    })
  ...
  describe("initiatePayment", function () {
    ...
    it("should succeed with an existing customer but no streampay id", async () => {
      const result = await streampayTest.initiatePayment(
        initiatePaymentContextWithExistingCustomer
      )

      expect(StreamPayMock.customers.create).toHaveBeenCalled()
      expect(StreamPayMock.customers.create).toHaveBeenCalledWith({
        email: initiatePaymentContextWithExistingCustomer.email,
      })

      expect(StreamPayMock.paymentIntents.create).toHaveBeenCalled()
      expect(StreamPayMock.paymentIntents.create).toHaveBeenCalledWith(
    ...
    it("should succeed with an existing customer with an existing streampay id", async () => {
      const result = await streampayTest.initiatePayment(
        initiatePaymentContextWithExistingCustomerStreamPayId
      )

      expect(StreamPayMock.customers.create).not.toHaveBeenCalled()
    ...
    it("should fail on customer creation", async () => {
      ...
      expect(StreamPayMock.paymentIntents.create).not.toHaveBeenCalled()
  ...
  describe("deletePayment", function () {
    ...
    it("should succeed", async () => {
      const result = await streampayTest.cancelPayment(deletePaymentSuccessData)
    ...
    it("should fail on intent cancellation", async () => {
      const result = await streampayTest.cancelPayment(deletePaymentFailData)
  ...
  describe("retrievePayment", function () {
    ...
    it("should fail on refund creation", async () => {
      const result = await streampayTest.retrievePayment(retrievePaymentFailData)
  ...
  describe("updatePayment", function () {
    ...
    it("should succeed to initiate a payment with an existing customer but no streampay id", async () => {
    ...
    it("should fail to initiate a payment with an existing customer but no streampay id", async () => {
      const result = await streampayTest.updatePayment(
        updatePaymentContextWithWrongEmail
      )
    ...
    it("should succeed but no update occurs when the amount did not changed", async () => {
      ...
      expect(StreamPayMock.paymentIntents.update).not.toHaveBeenCalled()
    ...
    it("should succeed to update the intent with the new amount", async () => {
      ...
      expect(StreamPayMock.paymentIntents.update).toHaveBeenCalled()
      expect(StreamPayMock.paymentIntents.update).toHaveBeenCalledWith(
